...
*Setup instructions for a Gitian build of Mountaincoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mountaincoin
...
- [Building Mountaincoin Core](#building-mountaincoin-core)
...
Clone the git repositories for mountaincoin and Gitian.
...
in the mountaincoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Mountaincoin Core
----------------

To build Mountaincoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mountaincoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mountaincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mountaincoin-project/mountaincoin
...
For example:
```bash
URL=https://github.com/thrasher-/mountaincoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mountaincoin=${COMMIT} --url mountaincoin=${URL} ../mountaincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mountaincoin=${COMMIT} --url mountaincoin=${URL} ../mountaincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mountaincoin=${COMMIT} --url mountaincoin=${URL} ../mountaincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mountaincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mountaincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mountaincoin
SIGPATH=/some/root/path/mountaincoin-detached-sigs

./bin/gbuild --url mountaincoin=${BTCPATH},signature=${SIGPATH} ../mountaincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mountaincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mountaincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mountaincoin-osx-build.assert
```
...
[mountaincoin-project/gitian.sigs.ltc](https://github.com/mountaincoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
