...
#if defined(NDEBUG)
# error "Mountaincoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Mountaincoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings >= 64)
        return 0;
    if (nHeight == 1) 
        nSubsidy = 6900000000 * COIN; 
    
    else   CAmount nSubsidy = 100 * COIN;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Mountaincoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
