...
const char * const BITCOIN_CONF_FILENAME = "mountaincoin.conf";
const char * const BITCOIN_PID_FILENAME = "mountaincoin.pid";
...
#else
    const char* pszModule = "mountaincoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mountaincoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mountaincoin";
#else
    // Unix
    return pathRet / ".mountaincoin";
#endif
#endif
}
